...
async with KaguraClient(api_key="kagura_...", mcp_url="https://...") as client:
    ...
    # touched by Sleep. Raw numbers here; prose ("hit goal weight") via remember().
...
|---|---|---|
| 0.40.0+ | 0.17.1 (0.54.0 for the measurement lane + list bbox) | **HOW-MUCH axis + WHERE-axis list filter.** `record_measurement()` / `recall_series()` and `kagura measure record\|series` need memory-cloud **0.54.0+** ([#1333](https://github.com/kagura-ai/memory-cloud/issues/1333)) — an older server returns "tool not found". `list_memories(lat_min=…, lat_max=…, lon_min=…, lon_max=…)` and `MemoryListItem.location` need **0.54.0+** too ([#1334](https://github.com/kagura-ai/memory-cloud/issues/1334)) — an older server **silently ignores** the bbox and returns an unfiltered page, and `location` stays `None`. The new parameters are keyword-only and omitted from the wire when unset, so existing calls are unchanged. `MIN_SERVER_VERSION` stays **0.17.1**. |
