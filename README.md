...
| Hermes Agent | `url` + `auth: oauth` | `hermes mcp add <name> --url <url> --auth oauth`, attached: its probe starts the browser sign-in. An entry Hermes saves without `auth: oauth` (it offers to continue without authentication when it cannot set up OAuth) counts as not saved | `hermes mcp login <name>` (the browser flow; the device flow waits on [memory-cloud#1671](https://github.com/kagura-ai/memory-cloud/issues/1671)) | `~/.hermes/mcp-tokens/<name>.json` (`$HERMES_HOME`, or the active Hermes profile's) |
