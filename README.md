...
`KaguraResponseError` from every `KaguraClient` method and the REST clients (a `KaguraError`
subclass; `.operation` names the MCP tool or `<Client>.<method>`, e.g.
`KaguraClient.get_server_info`, which `check_server_version` reports too) instead of a raw
pydantic `ValidationError` or `KeyError`. The message names the failing fields, not their
values. Upgrading `kagura-memory` is the likely fix. A network failure, a non-JSON 2xx body and
a non-2xx status other than 401/429 raise `KaguraConnectionError` instead.
...
|---|---|---|
| 0.42.0+ | 0.17.1 — checked against memory-cloud up to **0.77.0** | **Consistent client surface.** Drift — a 2xx body the SDK's model rejects — raises `KaguraResponseError` from the last six REST-backed `KaguraClient` methods too: `get_server_info`, `check_server_version` (`.operation` is `KaguraClient.get_server_info`), `get_embedding_status`, `get_memory_stats`, `find_duplicates` and `list_embedding_models` ([#277](https://github.com/kagura-ai/kagura-memory-python-sdk/issues/277)). SDKs up to 0.41.3 raised `KaguraConnectionError("Invalid response format: …")` there, with pydantic's text (payload values such as memory summaries included); the new message names the failing fields only. A network failure, a non-JSON 2xx body and a non-2xx status other than 401/429 still raise `KaguraConnectionError`, 401 `KaguraAuthError` and 429 `KaguraRateLimitError`; `except KaguraError` catches both. `kagura doctor` reports that drift as a failed `server` check that names `/api/v1/system/info` and suggests upgrading, instead of "Server unreachable" (exit code still 1), and a rate-limited probe (HTTP 429) is a failed check too, not a traceback. Client-side; any server. **`kagura update-memory --details/--location/--merge-details`** ([#247](https://github.com/kagura-ai/kagura-memory-python-sdk/issues/247)): the CLI can now revise a memory's `details`, which `remember --details/--location` could write but nothing on the CLI could change. `--details` and `--location` replace the details wholesale, as `update_memory(details=…)` does; `--merge-details` reads the memory with `reference()` first and merges the given top-level keys over the current ones, so `location`, `tool_trigger` and every other unmentioned key survive. Client-side; any server. On memory-cloud **0.78.0+** ([#1685](https://github.com/kagura-ai/memory-cloud/issues/1685)) a `reference` reply is bounded to `max_chars` (default 20,000) and leaves out `details` that do not fit (`details_omitted`); `--merge-details` then stops without writing and asks for the complete object with `--details`, because this SDK's `reference()` cannot page a bounded reply yet. `MIN_SERVER_VERSION` stays **0.17.1**. |
| 0.41.0+ | 0.17.1 — checked against memory-cloud up to **0.77.0**. Per-surface: 0.77.0 for `setup codex\|hermes\|openclaw --url-form --oauth` and `ServerInfo.terms_version` | **Checked against memory-cloud up to 0.77.0.** Every other surface keeps its floor from the rows below. `ServerInfo.terms_version` carries the deployment's terms-of-service version (memory-cloud **0.77.0+**, [#1665](https://github.com/kagura-ai/memory-cloud/issues/1665)); it is `None` when the deployment does not record acceptance, and on an older server. Terms acceptance is a web sign-in step and does not affect API or MCP calls. From 0.77.0 the REST tags route also sends the context name, so a `list_tags(with_tags=…)` drill-down is one request there ([#1669](https://github.com/kagura-ai/memory-cloud/issues/1669)); the SDK keeps it on REST for every server. **Harness OAuth URL form.** From 0.77.0 memory-cloud's OAuth dynamic client registration accepts Codex, Hermes Agent and OpenClaw clients on a loopback redirect ([#1657](https://github.com/kagura-ai/memory-cloud/issues/1657)), and `kagura setup codex\|hermes\|openclaw --url-form --oauth --mcp-url …` writes a URL entry with no key, which the harness then signs in to itself (each harness signed in end to end on a `/mcp/w/<workspace-id>` URL against memory-cloud 0.78.0, #284). Before it detects, runs or writes anything, setup sends an unauthenticated `GET /api/v1/system/info` to that server and stops (exit 1) on a version below 0.77.0 or one it cannot confirm. The stdio `kagura-mcp` entry stays the default ([trade-off](#codex-hermes-agent-and-openclaw)), and from memory-cloud 0.78.0 ([#1671](https://github.com/kagura-ai/memory-cloud/issues/1671)) Hermes can also sign in with `hermes mcp login <name> --flow device`, which needs no loopback callback. `MIN_SERVER_VERSION` stays **0.17.1**. |
...
(below) writes an entry with no key, which the harness then signs in to itself.
From memory-cloud 0.78.0
([memory-cloud#1671](https://github.com/kagura-ai/memory-cloud/issues/1671)), Hermes can also sign in with
its device flow, which needs no loopback callback.
...
the CLI on `PATH` (or, for Hermes and for Codex with `--oauth`, without a
terminal or with `-y`), it prints the block and the file to add it to and edits
nothing. When Hermes's
...
Each sign-in redirects the browser to the harness's loopback callback on the
harness host (the Sign in column names each harness's way round a browser that
cannot reach it), and memory-cloud's consent screen shows the client name the
...
registration on a loopback redirect. On 2026-09-25 each harness signed in end to
end on a `/mcp/w/<workspace-id>` URL against memory-cloud 0.78.0, through entries
`kagura setup … --url-form --oauth` wrote, and reached `tools/list`: Codex
`0.157.0`, Hermes Agent `v2026.9.24` (the browser sign-in and `hermes mcp login
--flow device`) and OpenClaw `2026.9.6`, on a WSL2 host with the browser on
Windows ([#284](https://github.com/kagura-ai/kagura-memory-python-sdk/issues/284)).
memory-cloud's protected-resource metadata names only `<FRONTEND_URL>/mcp`, which
the three accept for a `/mcp/w/<workspace-id>` URL. OpenClaw asks for every
scope the server lists, `memory:admin` and `memory:delete` included.

**stdio or OAuth?** The stdio entry stays the default:

| | stdio `kagura-mcp` (default) | `--url-form --oauth` |
|---|---|---|
| Sign-in | One `kagura auth login` (device flow, which works over SSH) serves every harness and the CLI | One browser sign-in per harness, each registering its own DCR client. The browser is redirected to the loopback callback on the harness host; when it cannot reach it, use Codex `mcp login --no-browser`, paste the redirect URL at Hermes's prompt (`hermes mcp login <name>`), or use OpenClaw `mcp login --code`. On memory-cloud 0.78.0+, Hermes's `mcp login <name> --flow device` needs no callback at all ([memory-cloud#1671](https://github.com/kagura-ai/memory-cloud/issues/1671)) |
