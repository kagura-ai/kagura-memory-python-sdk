...
async with KaguraClient(api_key="kagura_...", mcp_url="https://...") as client:
    ...
    # Optional response keys are omitted when empty (memory-cloud v0.73.0+) — use .get().
...
raw pydantic `ValidationError` or `KeyError`. Upgrading `kagura-memory` is the likely fix. The
`KaguraClient` methods that call REST endpoints — `get_server_info`, `check_server_version`,
`get_embedding_status`, `get_memory_stats`, `find_duplicates`, `list_memories`,
`list_embedding_models` — still raise `KaguraConnectionError` on drift, so catch `KaguraError`
to cover both.
...
Creating a resource, a resource token or a public context is plan-gated since memory-cloud
0.68.0 ([#1551](https://github.com/kagura-ai/memory-cloud/issues/1551)): on a plan without the
`resources` / `public_contexts` feature the server refuses the call. Resources, tokens and
public contexts that already exist keep serving.
