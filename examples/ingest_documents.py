#!/usr/bin/env python3
"""FileIngestor with non-PDF formats — text, HTML, DOCX, XLSX, PPTX, EPUB.

The ingestion pipeline picks a structural extractor from the file's MIME
type / extension, so the same ``FileIngestor.ingest()`` call works for any
supported format. Each document becomes one overview memory plus per-section
summaries (Markdown/HTML headings, Word heading styles, one section per
sheet/slide, PDF/EPUB outline), linked by ``declared_link`` edges.

Install only the extras you need (or ``ingest-all`` for everything), plus a
text-LLM key:

    pip install 'kagura-memory[ingest-all]'
    ...
    uv run python examples/ingest_documents.py ./notes.md
    uv run python examples/ingest_documents.py ./report.docx
    uv run python examples/ingest_documents.py ./data.xlsx
"""
...
async def main():
    ...
    source = sys.argv[1] if len(sys.argv) > 1 else "./notes.md"
    ...
    async with (
        ...
        # files_client archives the original bytes so download_url works later.
