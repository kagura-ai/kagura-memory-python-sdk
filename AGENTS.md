...
- **Client-surface parity** (v0.38.0, #241-#244): `KaguraClient.recall_nearby(context_id, lat, lon, radius_m=1000, k=20)` — deterministic WHERE-axis query mirroring `recall_upcoming`, returns `distance_m` (server v0.53.0+, memory-cloud#1331); `remember(supersedes=<uuid>)` shadows the old memory instead of destroying it like `forget`+`remember` (server v0.45.0+, memory-cloud#1208); `update_memory(details=...)` — **replaces details wholesale**, so round-trip `location` or it drops off `recall_nearby`; `list_tags(with_tags=[...])` server-side AND drill-down for faceted browsing (server v0.17.2+, memory-cloud#830). lat/lon must be JSON numbers — the server 422s string-typed coordinates by design
