...
- **Forward-tolerant responses** (v0.38.1, #250): server-growing value sets on response models are plain `str` — `SleepReport.status`/`RollbackResult.status` (incl. `degraded`, server v0.43.0+, with `SleepReport.llm_call_failures`), `IndexerState.job_status`, `IndexerStateMetrics.skipped_reason` (incl. `memories_per_day_exceeded`, server v0.68.0+); the `SleepRunStatus`/`IndexerJobStatus`/`IndexerSkippedReason` Literals only document the known values. A payload that still fails its model raises `KaguraResponseError` (`KaguraError` subclass, `.operation` = MCP tool name or `<Client>.<method>`), never a raw pydantic `ValidationError` — parse new responses via `parse_response` (MCP) / `self._parse` (REST)
